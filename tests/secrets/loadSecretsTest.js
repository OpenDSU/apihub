...
assert.callback('check if secrets endpoint encryption and key rotation work', async (callback) => {
    ...
    try {
        ...
        const ss3 = await SecretsService.getSecretsServiceInstanceAsync(folder);
        await ss3.rotateKeyAsync();
        const ss4 = await SecretsService.getSecretsServiceInstanceAsync(folder);
